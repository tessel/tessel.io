...
curl -O https://s3.amazonaws.com/builds.tessel.io/cli/tessel-cli-current.tar.gz
...
if [ $? -ne 0 ]; then
  ...
  sudo npm install -g --no-registry --loglevel=info --ignore-scripts tessel-cli-current.tar.gz 
  ...
  npm install -g --no-registry --loglevel=info --ignore-scripts tessel-cli-current.tar.gz 
fi
